/**
 * 重构类的原理: 让某个类的原型指向新的堆内存地址（重定向指向） 
 *     问题：重定向后的空间中不一定有constructor属性（只有浏览器默认给prototype开辟的堆内存中才存在constructor)，这样导致类和原型机制不完整: 所以需要我们手动再给新的原型空间设置
